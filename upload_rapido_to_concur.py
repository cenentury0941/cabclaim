...
from utils import get_ride_id_rapido, get_fare_amount_rapido, get_date_rapido

PDF_FOLDER = "workspace/rapido_receipts"
...
def main(
    ...
    for index, pdf_path in enumerate(pdf_files, start=1):
        ...
        ride_id = get_ride_id_rapido(doc)
        fare = get_fare_amount_rapido(doc)
        transaction_date = get_date_rapido(doc)

        doc.close()

        print(f"Date          : {transaction_date}")
        print(f"Fare          : {fare}")
        print(f"Number Plate  : {ride_id}")
